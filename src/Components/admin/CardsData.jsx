
const CardsData = [
    {
    id: 1,
    name: 'OnGoing Projects',
    cards:[
        {
            id: 1,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images: [
                { id: 1, src: "https://www.gettyimages.in/detail/video/growing-tree-stock-footage/2181662163" },
                { id: 2, src: "https://images.pexels.com/photos/4418939/pexels-photo-4418939.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 3, src: "https://images.pexels.com/photos/1261728/pexels-photo-1261728.jpeg" },
                { id: 4, src: "https://images.pexels.com/photos/1933239/pexels-photo-1933239.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 5, src: "https://images.pexels.com/photos/1323550/pexels-photo-1323550.jpeg" },
                { id: 6, src: "https://images.pexels.com/photos/1421903/pexels-photo-1421903.jpeg" },
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 2,
            name: 'Project 1',
            location:'gnmmmmmmmmmmmm',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images: [
                { id: 1, src: "https://www.gettyimages.in/detail/video/growing-tree-stock-footage/2181662163" },
                { id: 2, src: "https://images.pexels.com/photos/4418939/pexels-photo-4418939.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 3, src: "https://images.pexels.com/photos/1261728/pexels-photo-1261728.jpeg" },
                { id: 4, src: "https://images.pexels.com/photos/1933239/pexels-photo-1933239.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 5, src: "https://images.pexels.com/photos/1323550/pexels-photo-1323550.jpeg" },
                { id: 6, src: "https://images.pexels.com/photos/1421903/pexels-photo-1421903.jpeg" },
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 3,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images: [
                { id: 1, src: "https://www.gettyimages.in/detail/video/growing-tree-stock-footage/2181662163" },
                { id: 2, src: "https://images.pexels.com/photos/4418939/pexels-photo-4418939.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 3, src: "https://images.pexels.com/photos/1261728/pexels-photo-1261728.jpeg" },
                { id: 4, src: "https://images.pexels.com/photos/1933239/pexels-photo-1933239.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 5, src: "https://images.pexels.com/photos/1323550/pexels-photo-1323550.jpeg" },
                { id: 6, src: "https://images.pexels.com/photos/1421903/pexels-photo-1421903.jpeg" },
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 4,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images: [
                { id: 1, src: "https://www.gettyimages.in/detail/video/growing-tree-stock-footage/2181662163" },
                { id: 2, src: "https://images.pexels.com/photos/4418939/pexels-photo-4418939.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 3, src: "https://images.pexels.com/photos/1261728/pexels-photo-1261728.jpeg" },
                { id: 4, src: "https://images.pexels.com/photos/1933239/pexels-photo-1933239.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 5, src: "https://images.pexels.com/photos/1323550/pexels-photo-1323550.jpeg" },
                { id: 6, src: "https://images.pexels.com/photos/1421903/pexels-photo-1421903.jpeg" },
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 5,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images: [
                { id: 1, src: "https://www.gettyimages.in/detail/video/growing-tree-stock-footage/2181662163" },
                { id: 2, src: "https://images.pexels.com/photos/4418939/pexels-photo-4418939.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 3, src: "https://images.pexels.com/photos/1261728/pexels-photo-1261728.jpeg" },
                { id: 4, src: "https://images.pexels.com/photos/1933239/pexels-photo-1933239.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 5, src: "https://images.pexels.com/photos/1323550/pexels-photo-1323550.jpeg" },
                { id: 6, src: "https://images.pexels.com/photos/1421903/pexels-photo-1421903.jpeg" },
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 6,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images: [
                { id: 1, src: "https://www.gettyimages.in/detail/video/growing-tree-stock-footage/2181662163" },
                { id: 2, src: "https://images.pexels.com/photos/4418939/pexels-photo-4418939.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 3, src: "https://images.pexels.com/photos/1261728/pexels-photo-1261728.jpeg" },
                { id: 4, src: "https://images.pexels.com/photos/1933239/pexels-photo-1933239.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 5, src: "https://images.pexels.com/photos/1323550/pexels-photo-1323550.jpeg" },
                { id: 6, src: "https://images.pexels.com/photos/1421903/pexels-photo-1421903.jpeg" },
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 7,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images: [
                { id: 1, src: "https://www.gettyimages.in/detail/video/growing-tree-stock-footage/2181662163" },
                { id: 2, src: "https://images.pexels.com/photos/4418939/pexels-photo-4418939.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 3, src: "https://images.pexels.com/photos/1261728/pexels-photo-1261728.jpeg" },
                { id: 4, src: "https://images.pexels.com/photos/1933239/pexels-photo-1933239.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 5, src: "https://images.pexels.com/photos/1323550/pexels-photo-1323550.jpeg" },
                { id: 6, src: "https://images.pexels.com/photos/1421903/pexels-photo-1421903.jpeg" },
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 8,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images: [
                { id: 1, src: "https://www.gettyimages.in/detail/video/growing-tree-stock-footage/2181662163" },
                { id: 2, src: "https://images.pexels.com/photos/4418939/pexels-photo-4418939.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 3, src: "https://images.pexels.com/photos/1261728/pexels-photo-1261728.jpeg" },
                { id: 4, src: "https://images.pexels.com/photos/1933239/pexels-photo-1933239.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 5, src: "https://images.pexels.com/photos/1323550/pexels-photo-1323550.jpeg" },
                { id: 6, src: "https://images.pexels.com/photos/1421903/pexels-photo-1421903.jpeg" },
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 9,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images: [
                { id: 1, src: "https://www.gettyimages.in/detail/video/growing-tree-stock-footage/2181662163" },
                { id: 2, src: "https://images.pexels.com/photos/4418939/pexels-photo-4418939.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 3, src: "https://images.pexels.com/photos/1261728/pexels-photo-1261728.jpeg" },
                { id: 4, src: "https://images.pexels.com/photos/1933239/pexels-photo-1933239.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 5, src: "https://images.pexels.com/photos/1323550/pexels-photo-1323550.jpeg" },
                { id: 6, src: "https://images.pexels.com/photos/1421903/pexels-photo-1421903.jpeg" },
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 10,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images: [
                { id: 1, src: "https://www.gettyimages.in/detail/video/growing-tree-stock-footage/2181662163" },
                { id: 2, src: "https://images.pexels.com/photos/4418939/pexels-photo-4418939.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 3, src: "https://images.pexels.com/photos/1261728/pexels-photo-1261728.jpeg" },
                { id: 4, src: "https://images.pexels.com/photos/1933239/pexels-photo-1933239.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 5, src: "https://images.pexels.com/photos/1323550/pexels-photo-1323550.jpeg" },
                { id: 6, src: "https://images.pexels.com/photos/1421903/pexels-photo-1421903.jpeg" },
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 11,
            name: 'Project 1',
            images: [
                { id: 1, src: "https://www.gettyimages.in/detail/video/growing-tree-stock-footage/2181662163" },
                { id: 2, src: "https://images.pexels.com/photos/4418939/pexels-photo-4418939.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 3, src: "https://images.pexels.com/photos/1261728/pexels-photo-1261728.jpeg" },
                { id: 4, src: "https://images.pexels.com/photos/1933239/pexels-photo-1933239.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 5, src: "https://images.pexels.com/photos/1323550/pexels-photo-1323550.jpeg" },
                { id: 6, src: "https://images.pexels.com/photos/1421903/pexels-photo-1421903.jpeg" },
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 12,
            name: 'Project 1',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 13,
            name: 'Project 1',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 14,
            name: 'Project 1',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 15,
            name: 'Project 1',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        
        ]
    },
    {
    id: 2,
    name: 'Completed Projects',
    cards:[
        {
            id: 1,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images: [
                { id: 1, src: "https://images.pexels.com/photos/1001682/pexels-photo-1001682.jpeg?auto=compress&cs=tinysrgb&w=600" },
                { id: 2, src: "https://images.pexels.com/photos/4418939/pexels-photo-4418939.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1" },
                { id: 3, src: "https://images.pexels.com/photos/1054289/pexels-photo-1054289.jpeg?auto=compress&cs=tinysrgb&w=600" },
                { id: 4, src: "https://images.pexels.com/photos/1103970/pexels-photo-1103970.jpeg?auto=compress&cs=tinysrgb&w=600" },
                { id: 5, src: "https://images.pexels.com/photos/1227513/pexels-photo-1227513.jpeg?auto=compress&cs=tinysrgb&w=600" },
                { id: 6, src: "https://images.pexels.com/photos/1241368/pexels-photo-1241368.jpeg?auto=compress&cs=tinysrgb&w=600" },
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 2,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 3,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 4,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 5,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 6,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 7,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 8,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 9,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 10,
            name: 'Project 1',
            location:'ghflk;jasdfklha',
            owner:'lkjhlkajshdfkhj',
            consultant:'jkhkjlhskldf',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 11,
            name: 'Project 1',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 12,
            name: 'Project 1',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 13,
            name: 'Project 1',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 14,
            name: 'Project 1',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        {
            id: 15,
            name: 'Project 1',
            images:[
                {id:1,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:2,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:3,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:4,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:5,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                {id:6,src:"https://images.pexels.com/photos/302769/pexels-photo-302769.jpeg?auto=compress&cs=tinysrgb&w=600"},
                
            ],
            description: 'This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 v This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1 This is a project 1',
        },
        
        ]
    },
    {
    id: 3,
    name: 'Feedback Requests',
    cards:[
        {
            id: 1,
            email: 'project1@example.com',
            description: 'This project focuses on developing a user-friendly web application for e-commerce.',
        },
        {
            id: 2,
            email: 'project2@example.com',
            description: 'This project involves creating an AI-powered chatbot for customer service.',
        },
        {
            id: 3,
            email: 'project3@example.com',
            description: 'A mobile app designed to track fitness goals and monitor daily activities.',
        },
        {
            id: 4,
            email: 'project4@example.com',
            description: 'A data visualization tool for analyzing sales trends in real-time.',
        },
        {
            id: 5,
            email: 'project5@example.com',
            description: 'An educational platform offering interactive lessons for kids.',
        },
        {
            id: 6,
            email: 'project6@example.com',
            description: 'A blockchain-based solution for secure and transparent transactions.',
        },
        {
            id: 7,
            email: 'project7@example.com',
            description: 'An IoT system for smart home automation and energy efficiency.',
        },
        {
            id: 8,
            email: 'project8@example.com',
            description: 'A cloud-based CRM application for managing customer relationships effectively.',
        },
        {
            id: 9,
            email: 'project9@example.com',
            description: 'A platform for collaborative project management with real-time updates.',
        },
        {
            id: 10,
            email: 'project10@example.com',
            description: 'An online marketplace connecting freelancers with potential clients.',
        },
        {
            id: 11,
            email: 'project11@example.com',
            description: 'A cybersecurity tool to detect and mitigate potential threats in networks.',
        },
        {
            id: 12,
            email: 'project12@example.com',
            description: 'A weather forecasting app leveraging satellite data for accurate predictions.',
        },
        {
            id: 13,
            email: 'project13@example.com',
            description: 'A virtual reality game designed for immersive educational experiences.',
        },
        {
            id: 14,
            email: 'project14@example.com',
            description: 'An automated tool for generating financial reports and insights.',
        },
        {
            id: 15,
            email: 'project15@example.com',
            description: 'A social networking app built for professionals in the tech industry.',
        },
        {
            id: 16,
            email: 'project16@example.com',
            description: 'A platform for peer-to-peer lending with a secure payment gateway.',
        },
        {
            id: 17,
            email: 'project17@example.com',
            description: 'An AI-powered personal assistant for scheduling and task management.',
        },
    ]
    
    },
    {
    id: 4,
    name: 'Accepted Feedbacks',
    cards:[
        {
            id: 1,
            email: 'project1@example.com',
            description: 'This project focuses on developing a user-friendly web application for e-commerce.',
        },
        {
            id: 2,
            email: 'project2@example.com',
            description: 'This project involves creating an AI-powered chatbot for customer service.',
        },
        {
            id: 3,
            email: 'project3@example.com',
            description: 'A mobile app designed to track fitness goals and monitor daily activities.',
        },
        {
            id: 4,
            email: 'project4@example.com',
            description: 'A data visualization tool for analyzing sales trends in real-time.',
        },
        {
            id: 5,
            email: 'project5@example.com',
            description: 'An educational platform offering interactive lessons for kids.',
        },
        {
            id: 6,
            email: 'project6@example.com',
            description: 'A blockchain-based solution for secure and transparent transactions.',
        },
        {
            id: 7,
            email: 'project7@example.com',
            description: 'An IoT system for smart home automation and energy efficiency.',
        },
        {
            id: 8,
            email: 'project8@example.com',
            description: 'A cloud-based CRM application for managing customer relationships effectively.',
        },
        {
            id: 9,
            email: 'project9@example.com',
            description: 'A platform for collaborative project management with real-time updates.',
        },
        {
            id: 10,
            email: 'project10@example.com',
            description: 'An online marketplace connecting freelancers with potential clients.',
        },
        {
            id: 11,
            email: 'project11@example.com',
            description: 'A cybersecurity tool to detect and mitigate potential threats in networks.',
        },
        {
            id: 12,
            email: 'project12@example.com',
            description: 'A weather forecasting app leveraging satellite data for accurate predictions.',
        },
        {
            id: 13,
            email: 'project13@example.com',
            description: 'A virtual reality game designed for immersive educational experiences.',
        },
        {
            id: 14,
            email: 'project14@example.com',
            description: 'An automated tool for generating financial reports and insights.',
        },
        {
            id: 15,
            email: 'project15@example.com',
            description: 'A social networking app built for professionals in the tech industry.',
        },
        {
            id: 16,
            email: 'project16@example.com',
            description: 'A platform for peer-to-peer lending with a secure payment gateway.',
        },
        {
            id: 17,
            email: 'project17@example.com',
            description: 'An AI-powered personal assistant for scheduling and task management.',
        },
    ]
    },
]

export default CardsData;